...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  // Dedicated role-based navigation menus
  const adminNavItems: NavItem[] = [
    { label: "نظرة عامة للنظام", href: "/admin", icon: ShieldCheck },
    { label: "إدارة الأطباء والعيادات", href: "/doctors", icon: Stethoscope },
    { label: "التحليلات واستهلاك الـ AI", href: "/analytics", icon: BarChart3 },
    { label: "إعدادات المنصة والـ Meta", href: "/settings", icon: Settings },
  ];

  const doctorNavItems: NavItem[] = [
    { label: "الرئيسية", href: "/doctor", icon: LayoutDashboard },
    { label: "جدول المواعيد", href: "/appointments", icon: Calendar },
    { label: "سجل المرضى", href: "/patients", icon: Users },
    { label: "محادثات المرضى", href: "/conversations", icon: MessageSquare },
    { label: "محاكي الواتساب", href: "/simulator", icon: Smartphone, badge: "مباشر" },
    { label: "خدمات وأسعار العيادة", href: "/services", icon: Sparkles },
    { label: "الأسئلة الشائعة الـ FAQ", href: "/faqs", icon: HelpCircle },
    { label: "التذكيرات الآلية", href: "/reminders", icon: Bell },
    { label: "التحليلات وتكلفة الـ AI", href: "/analytics", icon: BarChart3 },
    { label: "إعدادات العيادة", href: "/settings", icon: Settings },
  ];

  const staffNavItems: NavItem[] = [
    { label: "لوحة الاستقبال", href: "/staff", icon: LayoutDashboard },
    { label: "مواعيد اليوم وحجز Walk-in", href: "/appointments", icon: Calendar },
    { label: "سجل المرضى والبحث", href: "/patients", icon: Users },
    { label: "محادثات المرضى", href: "/conversations", icon: MessageSquare },
  ];
  ...
  return (
    <div className="min-h-screen flex bg-[#0b132b] text-slate-100 dir-rtl">
      {/* Sidebar Desktop */}
      <aside className="hidden lg:flex flex-col w-72 border-l border-slate-800 bg-[#0f172a]/90 backdrop-blur-xl p-4 shrink-0">
        ...
        {/* Current Active User / Clinic Info */}
        <div className="mb-6 p-3 rounded-xl bg-slate-800/60 border border-slate-700/50 flex items-center gap-3">
          ...
          <div className="overflow-hidden">
            <p className="text-[10px] text-slate-400">
              {user?.role === "SUPER_ADMIN" ? "مدير النظام العام" : user?.role === "STAFF" ? "مكتب الاستقبال" : "حساب الطبيب الحالي"}
            </p>
        ...
        <div className="pt-4 border-t border-slate-800 mt-auto flex items-center justify-between px-2">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="overflow-hidden">
              <p className="text-xs font-semibold text-white truncate">{user?.name || "مستخدم"}</p>
        ...
        <header className="h-16 border-b border-slate-800 bg-[#0f172a]/70 backdrop-blur-md px-4 lg:px-8 flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3 relative">
            {user?.role !== "STAFF" && (
              <Link
                href="/simulator"
                className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-teal-500/10 border border-teal-500/30 text-teal-300 hover:bg-teal-500/20 text-xs font-semibold transition-colors"
              >
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700 text-xs font-bold text-white transition-all"
              >
                <div className="w-6 h-6 rounded-full bg-brand-600 text-white flex items-center justify-center text-[10px] font-bold">
                  {user?.name?.[0] || "ط"}
                </div>
                <span className="max-w-[100px] truncate">{user?.name || "المستخدم"}</span>
                <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
              </button>

              {userDropdownOpen && (
                <div className="absolute left-0 mt-2 w-48 bg-[#0f172a] border border-slate-800 rounded-2xl p-2 shadow-2xl z-50 space-y-1 dir-rtl">
                  ...
                  <Link
                    href={user?.role === "SUPER_ADMIN" ? "/admin" : user?.role === "STAFF" ? "/staff" : "/doctor"}
                    onClick={() => setUserDropdownOpen(false)}
                    aria-label="الانتقال إلى الرئيسية"
                    className="w-full text-right px-3 py-2 rounded-xl text-xs text-slate-300 hover:text-white hover:bg-slate-800 flex items-center gap-2"
                  >
                  ...
                  {user?.role !== "STAFF" && (
                    <Link
                      href="/settings"
                      onClick={() => setUserDropdownOpen(false)}
                      aria-label="الانتقال إلى إعدادات العيادة"
                      className="w-full text-right px-3 py-2 rounded-xl text-xs text-slate-300 hover:text-white hover:bg-slate-800 flex items-center gap-2"
                    >
                  ...
                  <button
                    onClick={handleLogout}
                    className="w-full text-right px-3 py-2 rounded-xl text-xs text-rose-400 hover:bg-rose-500/10 flex items-center gap-2"
                  >
